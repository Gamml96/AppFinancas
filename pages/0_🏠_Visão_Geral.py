...
# --- saldo atual consolidado considera só filtro de conta ---
if df.empty:
    ...
    saldo_atual_valor = fluxo_tudo[
        fluxo_tudo['data'].dt.date <= utils.get_local_today()
    ]['saldo_acumulado'].iloc[-1] if not fluxo_tudo.empty else 0.0
    st.metric("Saldo Atual Consolidado (Hoje)", utils.formatar_moeda_brl(saldo_atual_valor))
    st.markdown("---")

    # --- Usa apenas df_filtrado para gráfico e tabela detalhada ---
