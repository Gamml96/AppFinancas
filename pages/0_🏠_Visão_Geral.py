...
# Busca contas e categorias para os filtros
...
all_categorias = ([cat[1] for cat in categorias_receita] + [cat[1] for cat in categorias_despesa])
...
with filtro_cols[3]:
    cat_filtro = st.selectbox("Categoria", options=["Todas"] + all_categorias)
...
# ==== filtro de categoria no dataframe ====
df_filtrado = df.copy()
if cat_filtro != "Todas":
    df_filtrado = df_filtrado[df_filtrado["descricao"].str.contains(cat_filtro, case=False, na=False)]
...
if df_filtrado.empty:
    ...
    df_filtrado['entradas'] = df_filtrado['valor'].apply(lambda x: x if x > 0 else 0)
    df_filtrado['saidas'] = df_filtrado['valor'].apply(lambda x: abs(x) if x < 0 else 0)
    fluxo_diario = df_filtrado.groupby('data').agg(
    ...
    if not fluxo_diario.empty:
        ...
        saldo_atual_valor = fluxo_diario[
            fluxo_diario['data'].dt.date <= utils.get_local_today()
        ]['saldo_acumulado'].iloc[-1] if not fluxo_diario.empty else 0.0
        ...
        styled_df = df_display.style.apply(highlight_today, axis=1).format(
            {"entradas": utils.formatar_moeda_brl, "saidas": utils.formatar_moeda_brl, "saldo_acumulado": utils.formatar_moeda_brl}
        ).hide(axis="index")
        st.dataframe(
            styled_df,
            column_config={
        ...
        st.info("Não há dados para o período/conta/categoria filtrado selecionado.")
